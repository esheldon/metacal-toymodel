...
parser.add_argument('--mmodel', type=float, default=0.6, help='model bias')
...
@jit(nopython=True, cache=True)
def add_err_trunc_old(g1, g2, noise):
    """
...
@jit(nopython=True, cache=True)
def add_err_trunc(g1, g2, err1, err2):
    """
    truncated gaussian for errors
    """
...
def add_err_shear(g1, g2, noise):
    ...
    #return tg1, tg2, err1, err2
    return tg1, tg2



@jit(nopython=True, cache=True)
def get_biased_shape(g1, g2, mmodel, err1, err2):
    """
    input g1,g2 is the true sheared shape

    model is gmeas = m*g + err
    """

    bg1 = mmodel*g1
    bg2 = mmodel*g2

    ng1, ng2 = add_err_trunc(bg1, bg2, err1, err2)
...
@jit(nopython=True, cache=True)
def go(n, mmodel, shapenoise, s1, s2, noise, step):
    ...
    for i in xrange(n):
        ...
        for ipair in xrange(2):
            ...
            g1_1p, g2_1p = shear_reduced(g1, g2,  step, 0.0)
            g1_1m, g2_1m = shear_reduced(g1, g2, -step, 0.0)

            # note noise is added *after*
            err1 = noise*numpy.random.randn()
            err2 = noise*numpy.random.randn()
            g1n, g2n       = get_biased_shape(g1, g2, mmodel, err1, err2)
            g1n_1p, g2n_1p = get_biased_shape(g1_1p, g2_1p, mmodel, err1, err2)
            g1n_1m, g2n_1m = get_biased_shape(g1_1m, g2_1m, mmodel, err1, err2)


            R1 = (g1n_1p - g1n_1m)/(2*step)
...
def main():
    ...
    print("calculating")
    tm=time.time()
    #g1mean,g1err, g2mean,g2err,R1,R1err = go(g1, g2, s1, s2, noise, 0.01)
